...
const auth = module.exports = {
  ...
  processJWT(req, res, next) {
    ...
    if (!appId || !validate.isUUID(appId)) {
      return res.status(400).send('Missing valid App id "app" in JWT header.');
    }
    const app = database._getApp(appId);
    if (!app) {
      return res.status(400).send('App defined in JWT header does not exist.');
    }
    // Verify web token
    try {
      if (!jws.verify(tokenString, token.header.alg, app.secret)) {
        return res.status(400).send('JWT has an incorrect signature.');
      }
    } catch (ex) {
      logger.error(`Failed to verify JSON web token: ${ex}`);
      return res.status(500).send('Failed to verify JSON web token, you may ' +
    ...
    if (typeof token.payload !== 'object') {
      try {
        ...
        logger.error(`Failed to parse encrypted JSON web token payload: ${ex}`);
    ...
    if (!subject) {
      return res.status(400).send('JWT is missing its "sub" property in its ' +
    ...
    let expiry = token.payload.exp = Number(token.payload.exp ||
      token.header.exp);
    if (isNaN(expiry)) {
      return res.status(400).send('JWT is missing its "exp" property in its ' +
        'payload and header. This is requred to define an end time for which ' +
