...
module.exports = new Promise((resolve, reject) => {
  // Read default config
  util.readYamlFile(path.resolve(__dirname, '../config.default.yml'))
   .then((defaultConfig) => {
    // Read user config
    util.readYamlFile(path.resolve(__dirname, '../config.yml'))
     .then(userConfig => {
      // Merge then resolve
      ...
      // Warn and resolve
      logger.warn(`Error while reading config.yaml: ${err}`);
      resolve(defaultConfig);
    });
  }, err => {
    // Fatal, could not read default config
    logger.fatal(`Failed to read default config, will not start: ${err}`);
    reject(err);
  });
});
